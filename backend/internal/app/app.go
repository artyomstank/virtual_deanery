...
func New(cfg *config.Config) (*App, error) {
	...
	adminRepo := postgres_repo.NewAdminRepo(pool)
	...
	aclHandler := handler.NewACLHandler(aclService, adminRepo, log)
	adminHandler := handler.NewAdminHandler(adminRepo, userService, log, cfg.BCryptCost)

	// Роутер
	corsOrigins := []string{"*"}
	httpRouter := router.NewRouter(userHandler, aclHandler, adminHandler, userService, jwtClient, log, corsOrigins)
