...
type UserRepository interface {
	...
	// List возвращает список пользователей с опциональной фильтрацией.
	// isActive и role — указатели для опциональной фильтрации (nil означает не фильтровать).
	List(ctx context.Context, isActive *bool, role string) ([]entity.User, error)
...
type RoleRepository interface {
	...
	// Используется при инвалидации кэша ACL после обновления прав.
	// Если роль не найдена — возвращает ошибку "role not found".
...
type ACLRepository interface {
	...
	// GetByRoleID загружает все ACL-записи для указанной роли по её ID
	// с JOIN на таблицы roles и resources — поле Resource в каждом ACLEntry заполнено.
	// Если записей нет — возвращает пустой слайс (не nil).
	GetByRoleID(ctx context.Context, roleID int) ([]entity.ACLEntry, error)
